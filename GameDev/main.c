...
void Update(GameObject_Triangle* obj, int objNum, int e) {
    // 회전 값 처리
    if (e == -1) {
        obj->Rotation -= 30; // 좌측 회전 (회전 각도 감소)
    }
    if (e == 1) {
        obj->Rotation += 30; // 우측 회전 (회전 각도 증가)
    }
    ...
    // 삼각형의 상단 꼭짓점 (여기서는 2번 인덱스를 상단으로 설정)
    Vector3 topVertex = { obj->Line[2].x, obj->Line[2].y, 1.0f };
    ...
    // 이동 처리
    if (e == 2) {  // 'w' 키: 전진
        // 삼각형의 중심을 계산 (세 점의 평균 위치)
        float centroidX = (obj->Line[0].x + obj->Line[1].x + obj->Line[2].x) / 3.0f;
        float centroidY = (obj->Line[0].y + obj->Line[1].y + obj->Line[2].y) / 3.0f;

        // topVertex에서 중심을 향하는 벡터 계산
        float dx = obj->Line[2].x - centroidX;
        float dy = obj->Line[2].y - centroidY;

        // 이 벡터를 회전 각도에 맞춰 회전시킴 (회전 적용)
        float radians = obj->Rotation * (3.14159265359f / 180.0f);  // 회전 각도를 라디안으로 변환
        float rotatedDx = dx * cosf(radians) - dy * sinf(radians);  // 회전된 x 방향
        float rotatedDy = dx * sinf(radians) + dy * cosf(radians);  // 회전된 y 방향

        // 전진
        obj->Position.x += rotatedDx * moveDistance;
        obj->Position.y += rotatedDy * moveDistance;

        // 삼각형의 각 점을 회전된 방향으로 전진
        for (int i = 0; i < 3; i++) {
            obj->Line[i].x += rotatedDx * moveDistance;
            obj->Line[i].y += rotatedDy * moveDistance;
        }
    }
